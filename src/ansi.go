...
func toAnsiString(color Color, offset int) string {
	col := int(color)
	ret := ""
	if col == -1 {
		...
		if num, delimiter, ansiCode = parseAnsiCode(ansiCode, delimiter); num != -1 {
			...
			case 0:
				...
				case 49:
					state.bg = -1
				case 1:
					state.attr = state.attr | Bold
				case 2:
					state.attr = state.attr | Dim
				case 3:
					state.attr = state.attr | Italic
				case 4:
					state.attr = state.attr | Underline
				case 5:
					state.attr = state.attr | Blink
				case 7:
					state.attr = state.attr | Reverse
				case 9:
					state.attr = state.attr | StrikeThrough
				case 22:
					state.attr = state.attr &^ Bold
					state.attr = state.attr &^ Dim
				case 23: // tput rmso
